...
const CreateCoach = () => {
  ...
  async function submitData(e) {
    ...
    if (data.signed_user._id) {
      ...
      await axios
        .post(`http://localhost:4000/api/coach/create`, formData)
        .then((res) => {
          if (res.status === 200) {
            navigate("/coach");
          }
      ...
      <div className="max-w-3xl w-10/12 md:w-3/5 flex flex-col bg-white dark:bg-slate-700 mt-10 p-3 space-y-3 shadow-lg">
        <form onSubmit={submitData}>
          <div className="border-white border-b-2 space-y-4">
            ...
            <div className="mb-4">{imagePreview && <img src={imagePreview} alt="" className="h-screen w-min" />}</div>
          </div>
          <div className="w-full flex justify-end my-4 space-x-4">
            ...
            <button type="submit" className="createPostBtn bg-white hover:bg-slate-200">
              Coach
            </button>
          </div>
        </form>
