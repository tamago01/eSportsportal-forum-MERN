...
const User = () => {
  ...
  useEffect(() => {
    async function fetchUser() {
      await axios
        .post(`http://localhost:4000/api/user/${id}`)
        .then((res) => {
    ...
    async function fetchUsersPost() {
      await axios
        .get(`http://localhost:4000/api/post/user/${id}`)
        .then((res) => {
      ...
      <div className="w-full flex justify-center ">
        <div className="w-full md:w-4/5 flex flex-col-reverse md:flex-row items-center md:items-start md:space-x-7  my-[5vh] justify-center ">
          <div className="w-5/6 md:max-w-3xl md:w-4/6 flex flex-col  items-center">
            <div className="w-full text-center text-xl bg-primary text-white">{userProfile && <h3 className="font-semibold py-2">{userProfile?.name}'s posts</h3>}</div>
            {userPost &&
              userPost.map((post) => {
